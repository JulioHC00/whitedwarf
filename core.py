...
def solve(
    ...
    def change_to_envelope(x, variables):
        q, M, t = variables
        T = t * T_c
        tf = (sc.hbar**2 / (2 * sc.m_e * sc.k) *
              (((3 * sc.pi**2 * rho_r * Y_e) / (sc.m_p)) * q)**(2. / 3))
        if T >= tf:
            envelope = 0
        else:
            envelope = 1
    ...
    pressure = solve_ivp(pressure,
                         [reduced_core.density[-1],
                          reduced_core.density[0]],
                         [p_f],
                         method=solver,
                         t_eval=np.flip(reduced_core.density))
    ...
    if graphs:
        ...
        ax[1, 0].plot(core.radius / R_sun, core.pressure,
                      label='dP/drho method')
        ...
        ax[1,
           0].hlines((core.density[-1] / (m_u * mu)) * sc.k * core.temperature[-1],
                     core.radius[0] / R_sun,
                     core.radius[-1] / R_sun,
                     color='green',
                     linestyle='-.',
        ...
        ax[1,
           1].plot(core.radius / R_sun,
                   core.temperature,
                   label='Core temperature',
                   linestyle='dotted',
                   color='orange')
        ax[1, 1].plot(core.radius /
                      R_sun, (sc.hbar**2 /
                              (2 *
                               sc.m_e *
                               sc.k) *
                              (((3 *
                                 ...
                                 Y_e) /
                                (sc.m_p)) *
                                  reduced_core.density)**(2. /
                                                          3)), label='Fermi Temperature')
