...
def calculate(M, T_o, R, Y_e_core, mu_core, X, Y, Z, core_class, C, solver = 'RK23', r_tol = 1e-3, a_tol = 1e-6, graphs = False, t_max = 100, alpha = 0, beta =0, storing_times = None, crys = True):
    '''
    ...
    if alpha == 0:
        def equations(t, T):
            ...
            for i, rho in enumerate(np.flip(cor.density)):
                gamma = (1/(4*sc.pi*sc.epsilon_0))*(((6*C+8*(1-C))*sc.e)**2/(sc.k*T))*(4*sc.pi*rho/(3*mu_ion_core*m_u))**(1/3)
                
                if gamma >= 171 and crys:
                    mass_energy = ((mass[0]-mass[i])*Cv+(mass[i])*Cv_crystal)/(mu_ion_core*m_u)
                    break
    ...
    cool = solve_ivp(equations, [0,t_max], [T_o], method = solver, rtol = r_tol, atol = a_tol, t_eval = storing_times)
    
    class evolution:
        time = cool.t*million_years*1000
        core_temperature = cool.y[0] 
        luminosity = (32/(3*8.5))*sc.sigma*(4*sc.pi*sc.G*M/kappa_o)*mu_envelope*m_u/(sc.k)*core_temperature**(3.5)/((6e-6*mu_e_core)**2)
        surface_temperature = (luminosity/(4*sc.pi*R**2*sc.sigma))**(1/4)
        
    if graphs:
        fig, ax = plt.subplots(2,2, dpi =200, figsize = (20,20))
        ax[0,0].plot(evolution.time/(1000*million_years), evolution.core_temperature)
        ax[0,0].set_xlabel('Billion years')
        ax[0,0].set_ylabel('Core temperature [K]')
        ax[0,0].grid()
        
        ax[0,1].plot(evolution.time/(1000*million_years), evolution.surface_temperature)
        ax[0,1].set_xlabel('Billion years')
        ax[0,1].set_ylabel('Surface temperature [K]')
        ax[0,1].grid()
        
        ax[1,0].plot(evolution.time/(1000*million_years), evolution.luminosity/L_sun)
...
def full_calculate(rho_core, T_core,Y_e_core, C, X, Y, Z, solver = 'RK23', r_tol = 1e-3, a_tol = 1e-6, graphs = False, t_max = 14, full_output = False, storing_times = None, alpha = 0, beta = 0, crys = True):
    '''
    Description
    -----------
    ...
    evolution = calculate(cor.mass[-1], cor.temperature[-1], env.radius[-1], Y_e_core, mu_core, X, Y, Z, cor, C, graphs = graphs, solver = solver, t_max = t_max, storing_times = storing_times, r_tol = r_tol, a_tol = a_tol, alpha=alpha, beta = beta, crys = crys)
