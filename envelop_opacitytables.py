...
def solve(rho_core, T_core, Y_e, filename, op_test = -1, graphs=False, message = True, x_max=-1, rho_r=1e4, R_r = 1e4, P_r = -1, T_r = -1, density_cutoff = -1, l = 1, density = True, solver = 'RK23'):
    ...
    def op(rho, T):
        kappa = opacity.interpolate(filename,T,rho)
        return kappa

    def envelope_equations(x, variables):
        q, M, t = variables
        kappa = op((q+q_o)*rho_r, T_r*(t+t_o))  
